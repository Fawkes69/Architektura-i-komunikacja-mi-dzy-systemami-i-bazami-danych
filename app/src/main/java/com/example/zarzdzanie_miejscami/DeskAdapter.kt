...
class DeskAdapter(private val desks: List<Desk>) : RecyclerView.Adapter<DeskAdapter.DeskViewHolder>() {
    ...
    inner class DeskViewHolder(itemView: android.view.View) : RecyclerView.ViewHolder(itemView) {
        ...
        fun bind(desk: Desk) {
            ...
            priceTextView.text = "${desk.pricePerDay} zł/dzień"
            ratingBar.rating = desk.rating.toFloat()

            // Ustaw ikonę ulubionego
            favoriteButton.setImageResource(
                if (desk.isFavorite) R.drawable.ic_favorite_filled else R.drawable.ic_favorite_outline
            )

            // Nasłuchuj klik na serce
            favoriteButton.setOnClickListener {
                // TODO: Implementacja dodania/usunięcia z ulubionych
            }

            // Nasłuchuj rezerwację
